...
**Questions source:** Static JS file — `src/games/firstbell/questions.js`. No database. No dynamic fetch.

**Question count in questions.js:** **233 total** across 7 categories:
| Category | Count |
|----------|-------|
| gk (India GK) | 38 |
| bollywood | 45 |
| cricket | 39 |
| science | 38 |
| food (Food & Daily Life) | 25 |
| modern (Modern India) | 23 |
| brain (Brain Teasers) | 25 |
| **Total** | **233** |
...
**Root-level question files (NOT used by any game code — pipeline in progress):**

| File | Count | Format | Status |
|------|-------|--------|--------|
| `SampleQuestions.js` | ~25,799 lines, large array | `{category, question, options[], correctAnswer, explanation, difficulty, tags[]}` | Orphaned — no importer |
| `GeneratedQuestions.js` | 0 bytes, empty | N/A | Empty placeholder |
| `existing_questions.json` | 1,597 entries | Array of **question text strings only** (no options/answers) | Deduplication reference list |
| `Batch500_v2.json` | 99 question objects | `{category, question, options[], correctAnswer, explanation, difficulty, tags[]}` | Generated, unused |
| `New500Batch.json` | 132 question objects | Same as above | Generated by `scripts/generateNew500.js`, unused |

**Root-level category breakdown (Batch500_v2 + New500Batch, broader than in-game):**
Categories used: `general_knowledge`, `india_politics`, `indian_history`, `bollywood`, `cricket`, `sports`, `science`, `science_traps`, `space`, `world_geography`, `cities`, `quotes`, `law_cases`, `mind_blown`, `weird_facts`, `food`, `technology`, `business`

**scripts/ (new, not yet run):**
- `scripts/fetchOTDB.js` — fetches from Open Trivia DB API, writes to `questions/raw/`
- `scripts/transformOTDB.js` — transforms raw OTDB format to `{category, question, options[], correctAnswer, explanation, difficulty, tags[], source, verified}` format
- `scripts/validateQuestions.js` — validates transformed JSON
- `scripts/generateNew500.js` — generated `New500Batch.json` (already run once)

**`questions/` directory:** newly created with `raw/` and `transformed/` subdirs. Currently only contains `.gitkeep` files. Listed in `.gitignore`.
...
**Is Firestore being used for questions?** **No.** All 233 in-game questions are bundled in the static JS file. There is no `/questions` Firestore collection. The OTDB pipeline scripts have been created but not yet run; no questions have been pushed to any database.
...
│       └── strings.test.js
├── scripts/                       # NEW — Node.js tooling (not bundled)
│   ├── fetchOTDB.js               # Fetches from Open Trivia DB API → questions/raw/
│   ├── transformOTDB.js           # Transforms raw → {category,question,options[],correctAnswer,...}
│   ├── validateQuestions.js       # Validates transformed JSON (6 checks + duplicate detection)
│   └── generateNew500.js          # Generated New500Batch.json (already run once)
├── questions/                     # NEW — gitignored, only .gitkeep tracked
│   ├── raw/.gitkeep               # Will hold otdb_*.json files after fetchOTDB.js runs
│   └── transformed/.gitkeep      # Will hold otdb_transformed.json after transform
├── tests/                         # Service unit tests
├── cypress/                       # E2E specs
├── SampleQuestions.js             # Large JS question array (~25k lines), unused by game
├── GeneratedQuestions.js          # Empty file (0 bytes), placeholder
├── existing_questions.json        # 1,597 question-text strings, deduplication reference
├── Batch500_v2.json               # 99 question objects (broader categories), unused
├── New500Batch.json               # 132 question objects, generated by generateNew500.js
...
| OTDB pipeline scripts exist but have never been run | `scripts/` | Low — pipeline not yet executed |
| Root-level question files (SampleQuestions.js, existing_questions.json, Batch500_v2.json, New500Batch.json) have no importer and are unused | project root | Low — orphaned content |
...
### Today (2026-03-06) — OTDB Question Pipeline
