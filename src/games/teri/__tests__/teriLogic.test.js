...
describe('applyShufflerScore', () => {
  ...
  test('same shuffler continues when the result stays within [0, 52]', () => {
    const result = applyShufflerScore({ ...base, currentScore: 20, roundPointsForGameLead: 8, isShufflerOnGameLeadTeam: true })
  ...
  test('shuffler on GameLead team, GameLead wins -> score decreases (the worked example: 5-8=-3, rotates and flips to 3)', () => {
    const result = applyShufflerScore({ ...base, roundPointsForGameLead: 8, isShufflerOnGameLeadTeam: true })
  ...
  test('shuffler on the DEFENDING team mirrors the sign: GameLead losing -16 becomes +16 for the shuffler', () => {
    const result = applyShufflerScore({ ...base, currentScore: 5, roundPointsForGameLead: -16, isShufflerOnGameLeadTeam: false })
  ...
  test('crossing above 52 hands the role to the shuffler\'s own partner, reset to 0, and records the burst', () => {
    const result = applyShufflerScore({ ...base, currentScore: 50, roundPointsForGameLead: -5, isShufflerOnGameLeadTeam: true })
  ...
  test('exactly 52 does not burst', () => {
    const result = applyShufflerScore({ ...base, currentScore: 60, roundPointsForGameLead: 8, isShufflerOnGameLeadTeam: true })
    expect(result).toEqual({ shufflerId: 'p0', score: 52, burstPlayerId: null })
  })

  test('exactly 0 does not rotate', () => {
    const result = applyShufflerScore({ ...base, currentScore: 8, roundPointsForGameLead: 8, isShufflerOnGameLeadTeam: true })
...
describe('checkGameWinner', () => {
  test('no bursts yet -> no winner', () => {
    expect(checkGameWinner([], turnOrder)).toBeNull()
  })

  test('only one member of a team burst -> no winner yet', () => {
    expect(checkGameWinner(['p0'], turnOrder)).toBeNull()
  })

  test('both members of team A (p0, p2) burst -> team B wins', () => {
    expect(checkGameWinner(['p0', 'p2'], turnOrder)).toBe('teamB')
  })
