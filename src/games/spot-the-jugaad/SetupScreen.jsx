...
export function SetupScreen({ state, dispatch, slug }) {
  ...
  function handleStart() {
    const puzzles = generateSessionPuzzles(difficulty, 10)
    dispatch({
      type: ACTIONS.START_SESSION,
      payload: { difficulty, puzzles, startTime: Date.now() }
    })
  }

  return (
    <div className={`min-h-screen ${theme.bg} ${theme.text} flex flex-col px-6 pt-10 pb-8`}>
      <h1 className={`text-3xl font-bold ${theme.text} mb-1`}>🔍 Spot the Jugaad</h1>
      <p className={`${theme.textMuted} text-sm mb-2`}>Find the odd one out — 10 puzzles</p>
      {personalBest !== null && (
