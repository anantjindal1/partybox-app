  ...
  return (
    <div className="flex flex-col gap-5 max-w-lg w-full mx-auto pt-2">
            ...
            <button
              ...
              className={`min-h-[44px] rounded-xl border-[1.5px] px-4 text-left font-semibold transition-colors ${
                selectedTags.includes(key)
                  ? 'bg-fuchsia text-onFuchsia border-fuchsia'
                  : 'bg-surfaceElevated text-textPrimary border-border hover:border-fuchsia/50'
              }`}
            >
              {label.en}
            </button>
      ...
      <button
        ...
        disabled={!canStart}
        className="min-h-[44px] rounded-xl bg-fuchsia text-onFuchsia font-bold text-base disabled:opacity-40 disabled:cursor-not-allowed transition-opacity"
      >
