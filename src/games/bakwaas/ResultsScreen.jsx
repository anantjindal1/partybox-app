...
export function ResultsScreen({
  ...
  const maxScore = Math.max(0, ...players.map((p) => cumulativeScores[p.id] ?? 0))
  const winners = players.filter((p) => (cumulativeScores[p.id] ?? 0) === maxScore && maxScore > 0)

  return (
    <div className="flex-1 flex flex-col px-4 sm:px-6 py-6 gap-6 max-w-lg w-full mx-auto">
      <div className="flex flex-col items-center gap-3 text-center">
        <div className="w-16 h-16 rounded-full border-[1.5px] border-fuchsia text-fuchsia flex items-center justify-center">
          <MedalIcon />
        </div>
        ...
        {ranked.map((p, i) => (
          <div
            ...
            className={`flex items-center justify-between px-4 py-3 rounded-xl border-[1.5px] ${
              p.id === myId ? 'border-fuchsia bg-fuchsia/10' : 'border-border bg-surfaceElevated'
            }`}
          >
            <span className="text-sm font-semibold text-textPrimary">
              {i + 1}. {p.name}{p.id === myId ? ' (You)' : ''}
            </span>
            <span className="text-sm font-bold text-fuchsia tabular-nums">{cumulativeScores[p.id] ?? 0}</span>
          </div>
        ))}
      </div>
      ...
      <div className="flex flex-col gap-3">
        {isHost ? (
          <button onClick={onRematch} className="min-h-[44px] rounded-xl bg-fuchsia text-onFuchsia font-bold">
            {t('rematch')} →
          </button>
        ) : (
          <button onClick={onRematchVote} className="min-h-[44px] rounded-xl bg-fuchsia text-onFuchsia font-bold">
