  ...
  return (
    <div className="flex-1 flex flex-col px-4 sm:px-6 py-6 gap-5 max-w-lg w-full mx-auto">
      <p className="text-xs text-textMuted uppercase tracking-wider text-center">
        Round {handNumber} — Trump: {SUIT_LABEL[trumpSuit]} — Called by {callerName}
      </p>
      ...
      <div className="flex flex-col gap-2">
        <div className="flex items-center justify-between px-4 py-3 rounded-xl border-[1.5px] border-border bg-surfaceElevated">
          <div className="flex flex-col">
            ...
            <span className="text-xs text-textMuted">{teamTricks.teamA} hands this round</span>
          </div>
          <div className="flex flex-col items-end">
            <span className="text-sm font-bold text-jade tabular-nums">{handsWonAfter.teamA} rounds</span>
        ...
        <div className="flex items-center justify-between px-4 py-3 rounded-xl border-[1.5px] border-border bg-surfaceElevated">
          <div className="flex flex-col">
            ...
            <span className="text-xs text-textMuted">{teamTricks.teamB} hands this round</span>
          </div>
          <div className="flex flex-col items-end">
            <span className="text-sm font-bold text-jade tabular-nums">{handsWonAfter.teamB} rounds</span>
      ...
      {shutoutActive && (
        <p className="text-center text-xs text-error">
          {teamNames(shutoutTeam === 'teamA' ? teamAIds : teamBIds)} reached {MATCH_TARGET} points, but the other team hasn't won a round yet —
          the match continues until they reach {SHUTOUT_EXTENSION_TARGET} or the other team wins one round.
        </p>
      )}
      ...
      {seatManagement && seatManagement.openSeats.length > 0 ? (
        ...
        >
          {matchWinner ? 'See Final Results →' : 'Next Round →'}
        </button>
      ) : (
