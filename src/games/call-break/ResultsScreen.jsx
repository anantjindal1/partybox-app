...
export function ResultsScreen({ players, cumulativeScores, myId, isHost, onRematch, onHome }) {
  ...
  // top score is a completely normal Call Break outcome, still a winner.
  ...
  return (
    <div className="flex-1 flex flex-col py-6 gap-6 max-w-lg w-full mx-auto">
      <div className="flex flex-col items-center gap-3 text-center">
        <div className="w-16 h-16 rounded-full border-[1.5px] border-turquoise text-turquoise flex items-center justify-center">
          <TrophyIcon />
        </div>
        ...
        {ranked.map((p, i) => (
          <div
            ...
            className={`flex items-center justify-between px-4 py-3 rounded-xl border-[1.5px] ${
              p.id === myId ? 'border-turquoise bg-turquoise/10' : 'border-border bg-surfaceElevated'
            }`}
          >
            <span className="text-sm font-semibold text-textPrimary">
              {i + 1}. {p.name}{p.id === myId ? ' (You)' : ''}
            </span>
            <span className="text-sm font-bold text-turquoise tabular-nums">
              {(cumulativeScores[p.id] ?? 0).toFixed(1)}
            </span>
          </div>
        ))}
      </div>
      ...
      <div className="flex flex-col gap-3">
        {isHost && (
          <button onClick={onRematch} className="min-h-[44px] rounded-xl bg-turquoise text-onTurquoise font-bold">
            Rematch →
          </button>
        )}
